...
public class BankService extends BaseBankService {
  ...
  protected void execute() throws Exception {
    ...
    sendSecureDTO(SocketComponent.GATEWAY, getResponseDTO(receivedDTO));
  }
  ...
  private DTO getResponseDTO(DTO receivedDTO) throws Exception {
    ...
    return operationHandlers.get(operation).run(authenticatedDTO);
  }

  @Override
  protected DTO redirectToDatabase(DTO dtoToRedirect) throws Exception {
  ...
  protected DTO getBalance(
    ...
    double balanceValue = getBalanceFromDatabase(
      authenticatedDTO.getUserId()
    );

    String formattedBalanceValue = ValueFormatter.
      formatToBrazilianCurrency(balanceValue);
    return new MessageDTO(
      "Seu saldo atual é de " + formattedBalanceValue + "."
    );
  }
  ...
  private double getBalanceFromDatabase(UUID userId) throws Exception {
    DTO dtoToSend = new AuthenticatedDTO(userId).setOperation(
      RemoteOperation.GET_BALANCE
    );
